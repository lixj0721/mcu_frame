...
#endif
int32_t small_vsprintf(char *buf, const char *fmt, va_list args)
{
    char *str;
...
#endif
{
    ...
    selected_digits = (type & LARGE) ? large_digits : small_digits;
    if (type & LEFT)
        type &= ~ZEROPAD;
    ...
    {
        while (num != 0)
            tmp[i++] = selected_digits[divide(&num, base)];
    }
