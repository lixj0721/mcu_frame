...
int32_t AdpIicInit(void)
{
    ...
    {
        cfg.GPIO_Pin = g_iicCfg[i].sclpin;
        GPIO_Init(g_iicCfg[i].sclport, &cfg);
        cfg.GPIO_Pin = g_iicCfg[i].sdapin;
        GPIO_Init(g_iicCfg[i].sdaport, &cfg);
    }
...
#define SCL_SET(id, n) g_iicCfg[id].sclport->BSRR |= (g_iicCfg[id].sclpin<<(16*(1-(n?1:0))))
#define SDA_SET(id, n) g_iicCfg[id].sdaport->BSRR |= (g_iicCfg[id].sdapin<<(16*(1-(n?1:0))))
#define SDA_GET(id)    ((g_iicCfg[id].sdaport->IDR & g_iicCfg[id].sdapin)?1:0)
