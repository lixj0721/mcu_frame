...
static int32_t AdcClkInit(void)
{
    ...
    {
        clk = (g_adcPeriphCfg[i].adc==ADC1)?RCC_APB2Periph_ADC1:
                ((g_adcPeriphCfg[i].adc==ADC2)?RCC_APB2Periph_ADC2:
                    ((g_adcPeriphCfg[i].adc==ADC3)?RCC_APB2Periph_ADC3:-1));
        if(clk==-1) return -1;
